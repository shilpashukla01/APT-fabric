...
/*
    ...
    var engines = {},
        ...
        defaultOptions = {
            ...
            //selectable: false,
            selector: (
            fabric.window.Sizzle || (fabric.window.jQuery &&
            function(query) {
                return jQuery(query);
            ...
            separate: 'words',
            // 'none' and 'characters' are also accepted
            textShadow: 'none'
        };
...
Cufon.registerEngine('canvas', (function() {
    ...
    return function(font, text, style, options, node, el) {
        ...
        - expandLeft - ((1 / scale * canvas.width) / 2) + (Cufon.fonts[font.family].offsetLeft || 0), -expandTop - (Cufon.textOptions.height / scale) / 2);
...
(function() {
    ...
    function animate(options) {
        ...
        function() {}, abort = options.abort ||
        function() {
            return false;
        }, easing = options.easing ||
        function(pos) {
            return (-Math.cos(pos * Math.PI) / 2) + 0.5;
        }, startValue = 'startValue' in options ? options.startValue : 0, endValue = 'endValue' in options ? options.endValue : 100;
...
(function(global) {
    ...
    areHostMethods(fabric.document.documentElement, 'addEventListener', 'removeEventListener') && areHostMethods(fabric.window, 'addEventListener', 'removeEventListener')),

        shouldUseAttachEventDetachEvent = (
        areHostMethods(fabric.document.documentElement, 'attachEvent', 'detachEvent') && areHostMethods(fabric.window, 'attachEvent', 'detachEvent')),
    ...
    if (shouldUseAddListenerRemoveListener) { /** @ignore */
    ...
    } else if (shouldUseAttachEventDetachEvent) { /** @ignore */
        addListener = function(element, eventName, handler) {
    ...
    function pointerX(event) {
        ...
        return event.pageX || ((typeof event.clientX != 'unknown' ? event.clientX : 0) + (docElement.scrollLeft || body.scrollLeft) - (docElement.clientLeft || 0));
    }
    ...
    function pointerY(event) {
        ...
        return event.pageY || ((typeof event.clientY != 'unknown' ? event.clientY : 0) + (docElement.scrollTop || body.scrollTop) - (docElement.clientTop || 0));
    }
...
(function() {
    ...
    if (supportsOpacity) { /** @ignore */
        setOpacity = function(element, value) {
            element.style.opacity = value;
            return element;
        };
    } else if (supportsFilters) { /** @ignore */
        setOpacity = function(element, value) {
...
(function(global) {
    ...
    function parseAttributes(element, attributes) {
        ...
        var value, parsed, parentAttributes = {};
        ...
        for (var i = instances.length; i--;) {
            ...
            if (/^url\(/.test(instanceFillValue)) {
                ...
                if (fabric.gradientDefs[gradientId]) {
                    instances[i].set('fill', fabric.Gradient.fromElement(fabric.gradientDefs[gradientId], ctx, instances[i]));
                }
            }
        }
    }
    ...
    function parseElements(elements, callback, options) {
        var instances = Array(elements.length),
            i = elements.length;
    ...
    var svgCache = {
         ...
         * @param {Function} callback
         */
        get: function(url, callback) { /* NOOP */
        },
    ...
    function createSVGFontFacesMarkup(objects) {
        ...
        if (markup) {
            markup = ['<defs>', '<style type="text/css">', '<![CDATA[', markup, ']]>', '</style>', '</defs>'].join('');
        }
    ...
    fabric.Gradient = {
        ...
        fromElement: function(el, ctx, instance) {
            ...
            var colorStopEls = el.getElementsByTagName('stop'),
                el, offset, colorStops = {},
                colorStopFromStyle, coords = {
...
(function(global) {
    ...
    fabric.Color.sourceFromRgb = function(color) {
        ...
        if (match) {
            return [
            parseInt(match[1], 10), parseInt(match[2], 10), parseInt(match[3], 10), match[4] ? parseFloat(match[4]) : 1];
        }
    };
...
(function(global) {
    ...
    extend(fabric.StaticCanvas.prototype, /** @scope fabric.StaticCanvas.prototype */ {
         ...
         */
        onBeforeScaleRotate: function(target) { /* NOOP */
        },
        ...
        _initCanvasElement: function(element) {
            if (typeof element.getContext === 'undefined' && typeof G_vmlCanvasManager !== 'undefined' && G_vmlCanvasManager.initElement) {
         ...
         */
        toSVG: function() {
            var markup = ['<?xml version="1.0" standalone="no" ?>', '<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 20010904//EN" ', '"http://www.w3.org/TR/2001/REC-SVG-20010904/DTD/svg10.dtd">', '<svg ', 'xmlns="http://www.w3.org/2000/svg" ', 'xmlns:xlink="http://www.w3.org/1999/xlink" ', 'version="1.1" ', 'width="', this.width, '" ', 'height="', this.height, '" ', 'xml:space="preserve">', '<desc>Created with Fabric.js ', fabric.version, '</desc>', fabric.createSVGFontFacesMarkup(this.getObjects())];
    ...
    var InteractiveMethods = /** @scope fabric.Canvas.prototype */
    {
        ...
        _shouldClearSelection: function(e) {
            var target = this.findTarget(e),
                activeGroup = this.getActiveGroup();
            return (!target || (
            target && activeGroup && !activeGroup.contains(target) && activeGroup !== target && !e.shiftKey));
        },
        ...
        _setupCurrentTransform: function(e, target) {
            var action = 'drag',
                corner, pointer = getPointer(e);

            if (corner = target._findTargetCorner(e, this._offset)) {
                action = (corner === 'ml' || corner === 'mr') ? 'scaleX' : (corner === 'mt' || corner === 'mb') ? 'scaleY' : 'rotate';
            }
        ...
        _finalizeDrawingPath: function() {
            ...
            var minX = utilMin(this._freeDrawingXPoints),
                ...
                ctx = this.contextTop,
                path = [],
                xPoint, yPoint, xPoints = this._freeDrawingXPoints,
...
/* If lockUniScaling is on for this object, don't show
        ...
        _drawSelection: function() {
            ...
            groupSelector.ex - ((left > 0) ? 0 : -left), groupSelector.ey - ((top > 0) ? 0 : -top), aleft, atop);
            ...
            groupSelector.ex + STROKE_OFFSET - ((left > 0) ? 0 : aleft), groupSelector.ey + STROKE_OFFSET - ((top > 0) ? 0 : atop), aleft, atop);
        },

        _findSelectedObjects: function(e) {
            var target, targetRegion, group = [],
                x1 = this._groupSelector.ex,
                y1 = this._groupSelector.ey,
                x2 = x1 + this._groupSelector.left,
                y2 = y1 + this._groupSelector.top,
                currentObject, selectionX1Y1 = new fabric.Point(min(x1, x2), min(y1, y2)),
        ...
        findTarget: function(e, skipGroup) {
            ...
            for (var i = this._objects.length; i--;) {
                console.log("for: "+this._objects[i].type);
            ...
            console.log("outside target for");
...
(function(global) {
    ...
    fabric.Object = fabric.util.createClass( /** @scope fabric.Object.prototype */ {
        ...
        setOptions: function(options) {
            var i = this.stateProperties.length,
                prop;
            while (i--) {
        ...
        transform: function(ctx) {
            ...
            ctx.scale(
            this.scaleX * (this.flipX ? -1 : 1), this.scaleY * (this.flipY ? -1 : 1));
        },
        ...
        getSvgTransform: function() {
            ...
            return ["translate(", toFixed(this.left, 2), " ", toFixed(this.top, 2), ")", angle !== 0 ? (" rotate(" + toFixed(angle, 2) + ")") : '', (this.scaleX === 1 && this.scaleY === 1) ? '' : (" scale(" + toFixed(this.scaleX, 2) + " " + toFixed(this.scaleY, 2) + ")")].join('');
        },
        ...
        get: function(property) {
            return (property === 'angle') ? this.getAngle() : this[property];
        },
        ...
        drawBorders: function(ctx) {
            ...
            ctx.strokeRect(~~ (-(w / 2) - padding) + 0.5, // offset needed to make lines look sharper
            ~~ (-(h / 2) - padding) + 0.5, ~~ (w + padding2), ~~ (h + padding2));
        ...
        drawCorners: function(ctx) {
            ...
            var size = this.cornersize,
                ...
                left = -(this.width / 2),
                top = -(this.height / 2),
                _left, _top, sizeX = size / this.scaleX,
        ...
        intersectsWithRect: function(selectionTL, selectionBR) {
            ...
            var intersection = fabric.Intersection.intersectPolygonRectangle([tl, tr, br, bl], selectionTL, selectionBR);
...
(function(global) {
    ...
    fabric.Line.fromElement = function(element, options) {
        ...
        var points = [
        parsedAttributes.x1 || 0, parsedAttributes.y1 || 0, parsedAttributes.x2 || 0, parsedAttributes.y2 || 0];
...
(function(global) {
    ...
    fabric.Triangle = fabric.util.createClass(fabric.Object, /** @scope fabric.Triangle.prototype */ {
        ...
        toSVG: function() {
            ...
            return '<polygon ' + 'points="' + points + '" ' + 'style="' + this.getSvgStyles() + '" ' + 'transform="' + this.getSvgTransform() + '" ' + '/>';
        }
    });
...
(function(global) {
    ...
    fabric.Polyline = fabric.util.createClass(fabric.Object, /** @scope fabric.Polyline.prototype */ {
        ...
        toSVG: function() {
            ...
            return ['<polyline ', 'points="', points.join(''), '" ', 'style="', this.getSvgStyles(), '" ', 'transform="', this.getSvgTransform(), '" ', '/>'].join('');
        },
...
(function(global) {
    ...
    fabric.Polygon = fabric.util.createClass(fabric.Object, /** @scope fabric.Polygon.prototype */ {
        ...
        toSVG: function() {
            ...
            return ['<polygon ', 'points="', points.join(''), '" ', 'style="', this.getSvgStyles(), '" ', 'transform="', this.getSvgTransform(), '" ', '/>'].join('');
        },
...
(function(global) {
    ...
    function segmentToBezier(cx, cy, th0, th1, rx, ry, sin_th, cos_th) {
        ...
        return (segmentToBezierCache[argsString] = [
        a00 * x1 + a01 * y1, a10 * x1 + a11 * y1, a00 * x2 + a01 * y2, a10 * x2 + a11 * y2, a00 * x3 + a01 * y3, a10 * x3 + a11 * y3]);
    }
    ...
    fabric.Path = fabric.util.createClass(fabric.Object, /** @scope fabric.Path.prototype */ {
        ...
        initialize: function(path, options) {
            ...
            this.path = fromArray ? path : path.match && path.match(/[a-zA-Z][^a-zA-Z]*/g);
        ...
        _render: function(ctx) {
            ...
            for (var i = 0, len = this.path.length; i < len; ++i) {
                ...
                case 'C':
                    ...
                    controlY = current[4];
                    ctx.bezierCurveTo(
                    current[1] + l, current[2] + t, controlX + l, controlY + t, x + l, y + t);
                    break;
                ...
                case 's':
                    ...
                    ctx.bezierCurveTo(
                    controlX + l, controlY + t, x + current[1] + l, y + current[2] + t, tempX + l, tempY + t);
                    x = tempX;
                    y = tempY;
                    break;
                ...
                case 'S':
                    ...
                    controlY = 2 * y - controlY;
                    ctx.bezierCurveTo(
                    controlX + l, controlY + t, current[1] + l, current[2] + t, tempX + l, tempY + t);
                    x = tempX;
                    y = tempY;
                    break;
                ...
                case 'q':
                    ...
                    current[1] + l, current[2] + t, x + l, y + t);
                    break;
        ...
        toSVG: function() {
            ...
            return ['<g transform="', this.getSvgTransform(), '">', '<path ', 'width="', this.width, '" height="', this.height, '" ', 'd="', path, '" ', 'style="', this.getSvgStyles(), '" ', 'transform="translate(', (-this.width / 2), ' ', (-this.height / 2), ')" />', '</g>'].join('');
        },
        ...
        _parseDimensions: function() {
            var aX = [],
                aY = [],
                previousX, previousY, isLowerCase = false,
                x, y;
...
(function(global) {
    ...
    fabric.PathGroup = fabric.util.createClass(fabric.Path, /** @scope fabric.PathGroup.prototype */ {
        ...
        toSVG: function() {
            var objects = this.getObjects();
            var markup = ['<g ', 'width="', this.width, '" ', 'height="', this.height, '" ', 'style="', this.getSvgStyles(), '" ', 'transform="', this.getSvgTransform(), '" ', '>'];
...
(function(global) {
    ...
    fabric.Group = fabric.util.createClass(fabric.Object, /** @scope fabric.Group.prototype */ {
        ...
        _calcBounds: function() {
            var aX = [],
                aY = [],
                minX, minY, maxX, maxY, o, width, height, i = 0,
...
(function(global) {
    ...
    fabric.Image = fabric.util.createClass(fabric.Object, /** @scope fabric.Image.prototype */ {
        ...
        _render: function(ctx) {
            ...
            ctx.drawImage(
            this.getElement(), -originalImgSize.width / 2, -originalImgSize.height / 2, originalImgSize.width, originalImgSize.height);
        },
...
(function(global) {
    ...
    fabric.Text = fabric.util.createClass(fabric.Object, /** @scope fabric.Text.prototype */ {
        ...
        _getSVGShadows: function(lineTopOffset, textLines) {
            var shadowSpans = [],
                j, i, jlen, ilen, lineTopOffsetMultiplier = 1;

            for (j = 0, jlen = this._shadows.length; j < jlen; j++) {
                for (i = 0, ilen = textLines.length; i < ilen; i++) {
                    if (textLines[i] !== '') {
                        var lineLeftOffset = (this._boundaries && this._boundaries[i]) ? this._boundaries[i].left : 0;
                        shadowSpans.push('<tspan x="', toFixed((lineLeftOffset + lineTopOffsetMultiplier) + this._shadowOffsets[j][0], 2), (i === 0 ? '" y' : '" dy'), '="', toFixed(lineTopOffset + (i === 0 ? this._shadowOffsets[j][1] : 0), 2), '" ', this._getFillAttributes(this._shadows[j].color), '>', fabric.util.string.escapeXml(textLines[i]), '</tspan>');
        ...
        _getSVGTextAndBg: function(lineTopOffset, textLeftOffset, textLines) {
            ...
            for (i = 0, len = textLines.length; i < len; i++) {
                if (textLines[i] !== '') {
                    ...
                    textSpans.push('<tspan x="', lineLeftOffset, '" ', (i === 0 ? 'y' : 'dy'), '="', toFixed(lineTopOffset * lineTopOffsetMultiplier, 2), '" ',
                    // doing this on <tspan> elements since setting opacity on containing <text> one doesn't work in Illustrator
                    this._getFillAttributes(this.fill), '>', fabric.util.string.escapeXml(textLines[i]), '</tspan>');
                    lineTopOffsetMultiplier = 1;
                } else {
                    // in some environments (e.g. IE 7 & 8) empty tspans are completely ignored, using a lineTopOffsetMultiplier
                    // prevents empty tspans
                ...
                textBgRects.push('<rect ', this._getFillAttributes(this.backgroundColor), ' x="', toFixed(textLeftOffset + this._boundaries[i].left, 2), '" y="', /* an offset that seems to straighten things out */
                toFixed((lineTopOffset * i) - this.height / 2, 2), '" width="', toFixed(this._boundaries[i].width, 2), '" height="', toFixed(this._boundaries[i].height, 2), '"></rect>');
            }
            return {
                textSpans: textSpans,
